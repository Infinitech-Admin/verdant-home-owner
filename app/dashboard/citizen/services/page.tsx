...
export default function ServicesPage() {
  ...
  return (
    <CitizenLayout>
      {/* Page Header */}
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-yellow-600 mb-2">All Services</h1>
        <p className="text-gray-700">Browse and apply for city services</p>
      </div>
      ...
      <div className="relative mb-8">
        ...
        <input
          ...
          className="w-full pl-12 pr-4 py-3 rounded-lg bg-white border border-gray-300 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:border-transparent"
        />
      </div>
      ...
      <div className="space-y-8">
        {filteredCategories.length > 0 ? (
          filteredCategories.map((category, idx) => (
            <div key={idx}>
              <h2 className="text-xl font-bold text-emerald-800 mb-4">{category.title}</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {category.services.map((service, serviceIdx) => (
                  <button
                    key={serviceIdx}
                    onClick={() => router.push(service.path)}
                    className="bg-emerald-100 border border-emerald-200 rounded-xl p-6 hover:shadow-lg hover:border-emerald-400 hover:bg-emerald-50 transition-all text-left group"
                  >
                    <div className="flex items-start gap-4">
                      <div className="w-14 h-14 bg-emerald-700 rounded-xl flex items-center justify-center text-3xl flex-shrink-0 group-hover:scale-110 transition-transform">
                        {service.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-emerald-900 mb-1 group-hover:text-emerald-700 transition-colors">
                          {service.name}
                        </h3>
